...
inline bool IsValidListID(int listid)
{
	...
	return (bool)active_lists[listid];
}
...
cell AMX_NATIVE_CALL SLE_list_Create(AMX* amx, cell* params)
{
	...
	switch (params[1])
	{
		...
		case INIT_LIST_COPY:
		{
			int sourcelist = static_cast<int>(params[3]);
...
{
	const int listid = params[1];
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
	if (!(bool)active_lists[listid]) return INVALID_LIST_ID;

	active_lists[listid].reset();
	unused_list_ids.push_back(listid);
	return 0;
}
...
cell AMX_NATIVE_CALL SLE_list_empty(AMX* amx, cell* params)
{
	...
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
	if (!(bool)active_lists[listid]) return INVALID_LIST_ID;
	return active_lists[listid]->container->back();
}
cell AMX_NATIVE_CALL SLE_list_size(AMX* amx, cell* params)
{
	...
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
	if (!(bool)active_lists[listid]) return INVALID_LIST_ID;
	return active_lists[listid]->container->size();
}
cell AMX_NATIVE_CALL SLE_list_max_size(AMX* amx, cell* params)
{
	...
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
	if (!(bool)active_lists[listid]) return INVALID_LIST_ID;
	return active_lists[listid]->container->max_size();
}
...
cell AMX_NATIVE_CALL SLE_list_front(AMX* amx, cell* params)
{
	...
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
	if (!(bool)active_lists[listid]) return INVALID_LIST_ID;
	return active_lists[listid]->container->front();
}
...
cell AMX_NATIVE_CALL SLE_list_assign2(AMX* amx, cell* params)
{
	...
	switch (params[2])
	{
		case INIT_LIST_ARRAY_COPY:
		{
			cell* addr = NULL;
			amx_GetAddr(amx, params[3], &addr);
			active_lists[listid]->container->assign(addr, addr + static_cast<size_t>(params[4]));
			break;
		}
		case INIT_LIST_FILL:
		{
			active_lists[listid]->container->assign(static_cast<size_t>(params[3]), params[4]);
			break;
		}
	}
	return 0;
...
cell AMX_NATIVE_CALL SLE_list_push_front(AMX* amx, cell* params)
{
	...
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
	if (!(bool)active_lists[listid]) return INVALID_LIST_ID;
	active_lists[listid]->container->push_front(params[2]);
	return 0;
}
cell AMX_NATIVE_CALL SLE_list_pop_front(AMX* amx, cell* params)
{
	...
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
	if (!(bool)active_lists[listid]) return INVALID_LIST_ID;
	active_lists[listid]->container->pop_front();
	return 0;
}
cell AMX_NATIVE_CALL SLE_list_push_back(AMX* amx, cell* params)
{
	...
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
	if (!(bool)active_lists[listid]) return INVALID_LIST_ID;
	active_lists[listid]->container->push_back(params[2]);
	return 0;
}
cell AMX_NATIVE_CALL SLE_list_pop_back(AMX* amx, cell* params)
{
	...
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
	if (!(bool)active_lists[listid]) return INVALID_LIST_ID;
	active_lists[listid]->container->pop_back();
	return 0;
}

/*
TODO cell AMX_NATIVE_CALL SLE_list_insert(AMX* amx, cell* params)
{
...
cell AMX_NATIVE_CALL SLE_list_swap(AMX* amx, cell* params)
{
	...
	const int listid2 = params[2];
	if (listid2 < 0 || listid2 > list_maxid) return INVALID_LIST_ID;
	if (!(bool)active_lists[listid2]) return INVALID_LIST_ID;
...
cell AMX_NATIVE_CALL SLE_list_unqiue(AMX* amx, cell* params) //TODO
{
	//TO:DO Pred
	const int listid = params[1];
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
...
cell AMX_NATIVE_CALL SLE_list_sort(AMX* amx, cell* params) //TODO
{
	//TO:DO Pred
	const int listid = params[1];
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
...
cell AMX_NATIVE_CALL SLE_list_reverse(AMX* amx, cell* params) //TODO
{
	//TO:DO Pred
	const int listid = params[1];
	if (listid < 0 || listid > list_maxid) return INVALID_LIST_ID;
...
cell AMX_NATIVE_CALL SLE_list_merge(AMX* amx, cell* params)
{
	...
	if (active_lists[listid]->templateid != active_lists[listid2]->templateid)		
		logprintf(" [WARNING] Merging lists of different types might lead to data corruption.");
